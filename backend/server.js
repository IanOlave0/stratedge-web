...
const handle = async (req, res) => {
  ...
  if (routeKey === 'POST /api/leads') {
    ...
    return json(res, 201, { id: result.insertId, estimatedTotal, status: 'Nuevo lead' });
  }
  ...
  if (req.method === 'PUT' && leadStatusMatch) {
    if (!requireAdmin(req, res)) return;
    const body = await readBody(req);
    const allowed = ['Nuevo lead', 'Contactado', 'En cotizacion', 'Cliente activo', 'Cerrado'];
...
const start = async () => {
  ...
  createServer((req, res) => {
    ...
    console.log(`API Stratedge en http://localhost:${PORT}`);
